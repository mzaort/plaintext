...
plaintext.View = function() {
  ...
  /**
   * @private
   * @type {!goog.structs.Map.<!string, Function(Object)>}
   */
  this.componentRenders_ = new goog.structs.Map();
 ...
 * 
 * @param {string} name component css class
 * @param {function()} generator component
 */
 ...
 * 
 * @param {!string} name JS class name to trigger instantiation of the component.
 * @param {!Function} render The constructor of the component.
 */
plaintext.View.registerRender = function(name, render) {
  var cssName = name.toLowerCase().replace(/\./g, '-');
  plaintext.View.getInstance().componentRenders_.set(cssName, render);
};
...
goog.scope(function() {
  ...
  View.prototype.createComponentInstances_ = function(root, defaultParams) {
    ...
    this.componentRenders_.forEach(function(render, name) {
      var elements = goog.dom.getElementsByTagNameAndClass(null, name, filterParentElement) || [];
      goog.array.forEach(elements, function(element) {
        var id = element.id;
        var param = defaultParams[id] || {};
        goog.mixin(param, goog.dom.dataset.getAll(element));
  ...
  View.prototype.registerLabeledElement = function(element) {
    if (goog.dom.classlist.contains(element, plaintext.View.Classes.COMPONENT)) {
      ...
      if (componentNames.length !== 1) {
        ...
        if (goog.string.startsWith(componentNames[0], plaintext.View.Classes.COMPONENT_RENDER_PREFIX)) {
          View.registerRender(componentNames[0], View.RENDERS[componentNames[0]]);
        } else {
